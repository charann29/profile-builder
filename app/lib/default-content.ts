...
export const renderProfile = (data: Partial<ProfileData>) => {
    ...
    const brandEntries = positions.length > 0
        ? positions.slice(0, 6)
        : brands.length > 0
            ? brands.slice(0, 6).map(b => ({ company: b.name || '', title: b.role || '', logo: b.logo, duration: b.duration }))
            : [];
      ...
      <div class="profile-details">
        ...
        <div class="social-links">
          ...
          ${socialLinks.website || socialLinks.companyWebsite
            ? `<a href="${socialLinks.website || socialLinks.companyWebsite}" class="website">${((socialLinks.website || socialLinks.companyWebsite) ?? '').replace(/^https?:\/\//, '')}</a>`
            : ''}
        </div>
      </div>
    </div>
  ...
  <div class="page-2-content">
    ...
    <div class="section-box awards-section">
      ...
      <ul class="awards-list">
        ${awards.map(a => `<li><span class="bold-green">${a.title}${a.year ? ` (${a.year})` : ''}:</span> ${a.organization ? `By ${a.organization}` : ''}</li>`).join('')}
      </ul>
      ${hasMediaFeatures ? `
